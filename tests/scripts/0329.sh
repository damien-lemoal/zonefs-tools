...
if [ $# == 0 ]; then
	echo "Sequential file dsync write (async, iovec)"
        exit 0
fi

echo "Check sequential file dsync write (async, iovec)"

zonefs_mkfs "$1"
zonefs_mount "$1"

# Check with O_DSYNC file open
echo "Check async writes with O_DSYNC and iovecs"
...
echo "Check writes with RWF_APPEND and iovecs"

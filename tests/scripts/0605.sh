...
if [ $# == 0 ]; then
	echo "Sequential file write IO error (errors=zone-offline, async)"
        exit 0
fi
...
write_file_async_err "${zonefs_mntdir}/seq/0" 4096 0
...
# File permission should have changed and the file system
...
dd of=/dev/null if="${zonefs_mntdir}/seq/0" iflag=direct bs=4096 count=1 && \
	exit_failed " --> read SUCCESS (should FAIL)"

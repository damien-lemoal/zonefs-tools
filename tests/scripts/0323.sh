...
if [ $# == 0 ]; then
	echo "Sequential file seq read (direct)"
        exit 0
fi
...
# Sync direct sequential read
fio --name=seq_rndrd --filename="$zonefs_mntdir"/seq/0 \
    --rw=read --ioengine=psync --direct=1 \
    --bs=131072 --verify=md5 --do_verify=1 \
    --continue_on_error=none || \
	exit_failed "fio sync direct sequential read FAILED"
...
# Async direct sequential read
fio --name=seq_rndrd --filename="$zonefs_mntdir"/seq/0 \
    --rw=read --ioengine=libaio --iodepth=8 \
    --direct=1 --bs=131072 --verify=md5 --do_verify=1 \
    --continue_on_error=none || \
	exit_failed "fio async direct sequential read FAILED"
